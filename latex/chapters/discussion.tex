...
For square root, the errors were the result being off by one for numbers close to a square number.

Changing the model size did result in a model with bigger vectors or more layers needing fewer training steps to achieve the same addition accuracy (\cref{fig:model_size_add}) but did not make a substantial difference in model capabilities (\cref{tbl:model_size}), meaning it still could not learn tasks such as 5-digit multiplication.
