\chapter{Arithmetic using GPT-2}
\label{methods}
...
We use a transformer model with mostly the same GPT-2 architecture described in \cref{gpt2}, with parameters $\dVocab=128, \nBlocks=6, \dHidden=384, \nHeads=6, \dHead=64$ (these parameters are explained in \cref{gpt2:parameters}).
This means we use the same model architecture and configuration parameters as in \cite{teaching} and thus get a model with about 11M parameters \citepage{3}{teaching}. This is considerably smaller than the versions of GPT-2 released by OpenAI with about 100M to 1B parameters (\cref{gpt2:parameters}).
...
We study the performance of transformer models on three different arithmetic operations: addition, multiplication and integer square root. Addition and multiplication are standard arithmetic operations and common building blocks of more complex operations like exponentiation and evaluation of polynomial functions. Their learnability by transformer models has been investigated in \cite{teaching} and \cite{visual} among others. Integer square root, on the other hand, has to our knowledge not been studied before in the context of transformer networks. This operation is explained in more detail in \cref{methods:ops:isqrt}.
For all operations and experiments, operands are represented as a sequence of decimal digits.
The training sample strings for each arithmetic operation are structured as follows:

\begin{itemize}
	\item \textbf{Addition:} Let $a, b$ be positive integers and let $c=a+b$. The model is then trained on strings of the form $a+b=c$.
	\item \textbf{Multiplication:} Let $a, b$ be positive integers and let $c=a \cdot b$. The model is then trained on sequences of the form $a \cdot b=c$.
	\item \textbf{Integer square root:} Let $k$ be a positive integer. We train the model of sequences of the form $x: k$ where $k$ is the integer square root of $x$.
\end{itemize}
...
In addition to addition and multiplication, we also study the learnability of computing square roots. Instead of the regular square root function with irrational values, we use integer square root. This is consistent with the addition and multiplication operations in that it is an integer to integer mapping, with exactly one correct, finite answer for each input number, yet being equivalent to approximating the regular square root function.

The integer square root function $\lfloor \sqrt{x} \rfloor$ is defined as $\max(\{ k \in \mathbb{N} | k^2 \leq x \})$.
A model that can compute the integer square root $\lfloor \sqrt{x} \rfloor$ for integers $x$ with a certain number of decimal digits can also approximate the regular square root function, as shown in this example about computing an approximation of $\sqrt{2}$:

\begin{align*}
	\sqrt{2} &= 10^{-5} \sqrt{2 \cdot 10^{10}} \\
	\lfloor \sqrt{2 \cdot 10^{10}} \rfloor &= 141421 \\
	10^{-5} \lfloor \sqrt{2 \cdot 10^{10}} \rfloor &= 1.41421 \\
...
The distribution from which training samples are picked can have a big impact on the performance of machine learning models. To find out how much of an improvement can be had on the arithmetic tasks studied by us just by changing the sampling method, we experimented with multiple sampling methods: Basic sampling, from-zero sampling, uniform digits sampling and uniform bits sampling.
With the exception of the sampling strategy experiments (\cref{setup:sampling}) where we compare each of the four sampling methods, we stick to basic sampling for all experiments.
The sampling methods are defined as follows:

\begin{itemize}
	\item \textbf{Basic sampling:} For the addition and multiplication tasks with $n$ digits, the operands $a$ and $b$ are independently sampled uniformly from the range $[{10}^{n-1}, 10^n - 1]$, which ensures both operands have exactly $n$ decimal digits. We train the models for $n=3$, $n=5$,  and $n=10$ operand digits, giving a sample space of approximate size $10^6$, $10^{10}$, $10^{20}$ respectively.
	
	For the integer square root operation with $n$ operand digits, we also sample the operand $x$ uniformly from the range $[{10}^{n-1}, 10^n - 1]$. However, because square root is a unary operation, we double the number of decimal digits of the single operand to make sure we have a similarly large space of possible samples. Therefore we train the models for $n=6$, $n=10$,  and $n=20$ operand digits, giving a sample space of approximate size $10^6$, $10^{10}$, $10^{20}$ respectively, similar to the sample space sizes for addition and multiplication.
	
	\item \textbf{From-zero sampling:} For basic sampling, the limits of operand sampling were set as $[10^{n-1}, 10^n - 1]$ to include all $n$-digit numbers, where n is the number of operand digits. From-zero sampling instead uses the limits $[0, 10^n - 1]$, thus also including operands with fewer than $n$ decimal digits.
	
	\item \textbf{Uniform digits sampling:} For this sampling method, first a number of decimal digits $k$ is picked uniformly at random from $[1, n]$. Then the operands are picked uniformly at random from $[10^{k-1}, 10^k - 1]$, to ensure they are $k$-decimal digit integers.
	
	\item \textbf{Uniform bits sampling:} Let $p$ be the smallest integer so that $2^p \geq {10}^n$. This ensures all numbers with $n$ decimal digits are covered by the range $[1, 2^p]$.
...
Each step is separated by  pipe(\verb!|!) characters. The two summand digits, previous carry, sum digit, and new carry are written for each step.
...
The final addition happens like this ($5360+603=5963$ for our example):
...
The model consists of a pre-processing stage, a number of GPT-2 blocks and finally a post-processing stage (\cref{diagrams/gpt2/overview}). Inside the GPT-2 blocks, the core part is the attention mechanism (\cref{gpt2:attn_dot}): The input vector at each position is transformed into a query, a key and a value vector. The query vector at one position is then compared with the key vectors at every position, to generate an attention score from 0 to 1 at each position, which is then used to blend the value vectors together to get the output at this position (\cref{diagrams/gpt2/attention_dot}).
...
We can thus generate, for a given token sequence, a diagram that, for each GPT-2 block and each attention head inside that block, contains a 2D heatmap that shows which token / position attends to which other position.
