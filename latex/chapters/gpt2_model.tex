...
Finally, softmax is applied to transform this vector $\modOut'_i$ into a proper probability distribution $\modOut_i$ (\cref{eq:postproc_softmax}).
...
\begin{align}
	...
	\\
	\modOut_i &= \operatorname{softmax}(\modOut'_i)
	\label{eq:postproc_softmax}
\end{align}
...
\includediagram{diagrams/gpt2/block}{Overview of the components of a GPT-2 block with index $\modLayer$. This example shows three positions $\modHiddenAt{\modLayer}_1 \ldots \modHiddenAt{\modLayer}_3$, but the actual model is flexible regarding sequence length.}
...
\begin{enumerate}
	...
	\item The concatenated attention outputs are passed through a linear transform with learnable parameters $(\mathbf{W}_{\mathrm{proj}}, \mathbf{v}_{\mathrm{proj}})$, as described by \cref{eq:mha:output}. This results in the output of the multi-head attention mechanism, $\modHidden'_i$.
\end{enumerate}
