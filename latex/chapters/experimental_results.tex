...
Based on these accuracy curves, it does not seem unreasonable that training for longer that early stopping permits (\cref{setup:main}) would have resulted in even better accuracy. Investigating the type and parameters of our early stopping criterion could be worthwhile.
...
For the square root task we can see in \cref{tbl:baseline_sqrt} that accuracy numbers that lie somewhere between those for addition and multiplication.
...
For 5-digit multiplication, for which the model did not achieve above-zero accuracy (\cref{tbl:baseline_mul}), the first and last digits were still correct for failed tasks, but multiple digits in the middle of the product were wrong (\cref{tbl:errors_mul_5digits}).
...
\newpage
\section{Model size experiments}
\label{results:modelsize}



For 10-digit addition, we can see in \cref{tbl:model_size} that all of the tried model configurations were able to learn the task successfully, although there are small differences in final accuracy, such as the $n_{embd}=768$ model finishing with perfect accuracy on the test dataset.
For 5-digit multiplication, we can see that none of the model size changes made the model capable of learning this harder task.
...
\section{Sampling strategy experiments}
...
\begin{table}[H]
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/basic_add.csv}{}
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/from_zero_add.csv}{}
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/uniform_digits_add.csv}{}
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/uniform_bits_add.csv}{}
...
\begin{table}[H]
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/basic_mul.csv}{}
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/from_zero_mul.csv}{}
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/uniform_digits_mul.csv}{}
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/uniform_bits_mul.csv}{}
...
\begin{table}[H]
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/basic_sqrt.csv}{}
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/from_zero_sqrt.csv}{}
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/uniform_digits_sqrt.csv}{}
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/sampling_strategies_final_accuracy/uniform_bits_sqrt.csv}{}
...
\section{Result reversal experiments}
...
\begin{table}[H]
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/intermediate_steps_final_accuracy/reverse/add.csv}{}
...
\begin{table}[H]
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/intermediate_steps_final_accuracy/reverse/mul.csv}{}
...
\begin{table}[H]
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/intermediate_steps_final_accuracy/reverse/sqrt.csv}{}
...
\subsection{Accuracy development}
...
For 5-digit multiplication (\cref{tbl:reverse_errors_mul_5digits}), we can also observe similar error patterns compared to the corresponding baseline experiment (\cref{tbl:errors_mul_5digits}): The first and last digits of the result are correct, while the digits in the middle are wrong.
...
\section{Scratchpad experiments}
\label{results:scratchpad}


For the addition task (\cref{tbl:scratchpad_add}), using a scratchpad brings clear improvements over baseline (\cref{tbl:scratchpad_add_baseline}): For the 1k dataset size, training was more or less successful for all operand sizes (good accuracy but not 100\%), whereas there was no accuracy for the baseline experiments at 1k dataset size.
...
\begin{table}[H]
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/intermediate_steps_final_accuracy/scratchpad/add.csv}{}
...
\begin{table}[H]
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/intermediate_steps_final_accuracy/scratchpad/mul.csv}{}
...
\begin{table}[H]
	...
	\begin{minipage}[t]{0.47\linewidth}
		\hfill
		\includeAccuracyTableCore{experiment_results/intermediate_steps_final_accuracy/scratchpad/sqrt.csv}{}
...
We only looked at three errors per task due to the longer output size when using the scratchpad. The error samples presented are simply the first three samples from the error analysis dataset where the model output did not match the correct output (considering both the scratchpad and final result).
...
Attention activation for the addition task "123+456=0579" is illustrated in \cref{fig:heatmap_add_full} for each of the six GPT-2 blocks and each of the six attention heads per block.
...
This pattern is more or less the same across all six attention heads.
...
Since we found out that the most visible attention patterns are in the first block, to keep the diagrams of reasonable size especially for larger sequences, for the rest of the diagrams only the attention mechanism of the first GPT-2 block is considered, and the attention values of all six heads are averaged.
...
\includePDFPlot{experiment_results/heatmaps/add_simple_1.pdf}{fig:heatmap_add_simple1}{First block attention heatmap for the addition task "123+456", averaged over all six heads.}
...
\includePDFPlot{experiment_results/heatmaps/add_simple_2.pdf}{fig:heatmap_add_simple2}{First block attention heatmap for the addition task "999+001", averaged over all six heads.}
...
\includePDFPlot{experiment_results/heatmaps/add_5digits_1.pdf}{fig:heatmap_add_5digits}{First block attention heatmap for the addition task "12345+67890", averaged over all six heads.}
...
\includePDFPlot{experiment_results/heatmaps/add_rev_1.pdf}{fig:heatmap_add_rev}{First block attention heatmap for the addition task "123+456", using result reversal, averaged over all six heads.}
...
\includePDFPlot{experiment_results/heatmaps/add_scratch_1.pdf}{fig:heatmap_add_scratch}{First block attention heatmap for the addition task "123+456", using scratchpad, averaged over all six heads.}



\newpage
\subsection{Multiplication task}
...
\includePDFPlot{experiment_results/heatmaps/mul_1.pdf}{fig:heatmap_mul}{First block attention heatmap for the multiplication task "123*456", averaged over all six heads.}



\newpage
\subsection{Square root task}
...
\includePDFPlot{experiment_results/heatmaps/sqrt_1.pdf}{fig:heatmap_sqrt}{First block attention heatmap for the integer square root task "123456", averaged over all six heads.}